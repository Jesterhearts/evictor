        ...
        if $index >= $queue.len() {
            ...
            if $queue.len() == 1 {
                ...
                if $index == $metadata.head {
                    ...
                    $metadata.head = e.next.unwrap_or_default();
                }
                ...
                if $index == $metadata.tail {
                    $metadata.tail = e.prev.unwrap_or_default();
                }
...
macro_rules! validate_ll {
    ($metadata:expr, $queue:expr) => {
        ...
        assert_eq!(
            keys,
            $queue.keys().collect::<std::collections::HashSet<_>>(),
            "LLPolicy debug validation failed: item count mismatch: {keys:?} {:#?} {:#?}",
            $metadata,
            $queue
        );
        if keys.is_empty() {
        ...
        for key in keys {
            assert!($queue.contains_key(key), "LLPolicy debug validation failed: key {key:?} not found in queue: {:#?} {:#?}", $metadata, $queue);
        }
    };
}
